<template>
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['uav:route-point:create']"
        >
        ...
        <el-button
          ...
          v-hasPermi="['uav:route-point:export']"
        >
        ...
        <el-button
            ...
            v-hasPermi="['uav:route-point:delete']"
        >
  ...
  <ContentWrap>
    ...
    <el-table-column type="selection" width="55" />
      ...
      <el-table-column label="点位序号" align="center" prop="seqNo" />
...
<!--      <el-table-column label="动作参数" align="center" prop="actionParam" />-->
<!--      <el-table-column label="备注" align="center" prop="remark" />-->
      ...
      <el-table-column label="操作" align="center" min-width="120px">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['uav:route-point:update']"
          >
            编辑
          </el-button>
          <el-button
  ...
  <!-- 表单弹窗：添加/修改 -->
  <RoutePointForm ref="formRef" @success="getList" />
</template>
...
import RoutePointForm from './RoutePointForm.vue'
import { Route, RouteApi } from "@/api/uav/route"
import { ref } from "vue"

/** 航线点位 列表 */
defineOptions({ name: 'RoutePoint' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<RoutePoint[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await RoutePointApi.getRoutePointPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await RoutePointApi.deleteRoutePoint(id)
...
const handleDeleteBatch = async () => {
  try {
    // 删除的二次确认
    await message.delConfirm()
    await RoutePointApi.deleteRoutePointList(checkedIds.value);
    checkedIds.value = [];
    message.success(t('common.delSuccess'))
    await getList();
  } catch {}
...
const handleRowCheckboxChange = (records: RoutePoint[]) => {
...
const handleExport = async () => {
  try {
    ...
    const data = await RoutePointApi.exportRoutePoint(queryParams)
    download.excel(data, '航线点位.xls')
  } catch {
  } finally {
...
onMounted(() => {
  getList()
  RouteApi.getRouteSelect().then(res => {
