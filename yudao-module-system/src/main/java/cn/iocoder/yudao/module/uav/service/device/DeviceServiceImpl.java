...
import cn.iocoder.yudao.module.uav.dal.mysql.device.DeviceMapper;
...
public class DeviceServiceImpl implements DeviceService {
    ...
    private void validateDeviceExists(Long id) {
        if (deviceMapper.selectById(id) == null) {
            throw exception(new ErrorCode(401, "设备不存在"));
        }
    }
    ...
    @Override
    public List<DeviceDO> getDeviceSelect() {
        return deviceMapper.selectList(new LambdaQueryWrapperX<DeviceDO>()
                .select(DeviceDO::getId, DeviceDO::getDeviceCode, DeviceDO::getDeviceName)
                .eq(DeviceDO::getDeleted, false)
        );
    }
}